...
const login = async (email, password) => {
  ...
  return { token, user: { id: user.rows[0].id, email: user.rows[0].email, full_name: user.rows[0].full_name } };
};
...
const createGoogleUser = async (email, fullName) => {
  const result = await pool.query(
    ...
    [email, fullName, "GOOGLE_ACCOUNT"]
  );
  return result.rows[0];
};

module.exports = { register, login, findByEmail, createGoogleUser };